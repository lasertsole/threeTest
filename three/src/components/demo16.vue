...
import { onMounted, ref } from "vue";
  ...
  //导入纹理
  const textureLoader = new THREE.TextureLoader(loadingManager);//将加载管理器导入纹理加载器
  ...
  const metalnessTexture = textureLoader.load('./MOYA.jpg');//导入金属图
  const normalTexture = textureLoader.load("./MOYA.jpg");//导入法线贴图
